...
def getAngle(p1, p2):
    ...
    B = np.array([origin[0] + 1, origin[1]])

    dotpr = np.dot(A - origin, B - origin)
    angle = dotpr / np.linalg.norm(A - origin) / np.linalg.norm(B - origin)
...
def draw_humans(img, human_list):
    ...
    for human in human_list:
        ...
        color = [0, 255, 0]
        if (8 in centers.keys() and 11 in centers.keys()):
            if abs(getAngle(centers[8], centers[11])) < 0.8:
                color = [0, 0, 255]

        if (8 in centers.keys() and 1 in centers.keys()):
            if abs(getAngle(centers[8], centers[1])) > 0.6:
                color = [0, 0, 255]
            # if centers[8][1] <= centers[1][1]:
            #     color = [0, 0, 255]

        if (1 in centers.keys() and 11 in centers.keys()):
            if abs(getAngle(centers[11], centers[1])) > 0.6:
                color = [0, 0, 255]
            # if centers[11][1] <= centers[1][1]:
            #     color = [0, 0, 255]

        if (8 in centers.keys() and 11 in centers.keys()):
            img_copied = cv2.line(img_copied, centers[8], centers[11], color, 2)

        if (8 in centers.keys() and 1 in centers.keys()):
            img_copied = cv2.line(img_copied, centers[8], centers[1], color, 2)

        if (1 in centers.keys() and 11 in centers.keys()):
            img_copied = cv2.line(img_copied, centers[1], centers[11], color, 2)
